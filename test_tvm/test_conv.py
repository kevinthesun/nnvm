...
def end2end_benchmark():
    ...
    data_array = np.reshape(data_array,(1, 512, 16, 16))
    ...
    sym = mx.sym.Convolution(data, kernel=(1, 1), pad=(0, 0), num_filter=128, name="conv1_2", no_bias=True)
    mod = mx.mod.Module(symbol=sym, context=mx.cpu(), label_names=None)
    mod.bind(for_training=False, data_shapes=[('data', (1, 512, 16, 16))],
    ...
    tvm_out = module.get_output(0, out=tvm.nd.empty((1, 2, 16, 16, 64)))
    np_tvm_out = np.transpose(tvm_out.asnumpy(), (0, 1, 4, 2, 3))
    np_tvm_out = np.reshape(np_tvm_out, (1, 128, 16, 16))
    #print(tvm_out.shape)
    print(tvm_time)
